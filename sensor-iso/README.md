...
### <a name="arkime-capture"></a>arkime-capture: Arkime session forwarding
...
Finally, you’ll be given the opportunity to review the all of the arkime-capture forwrading options you’ve specified. Selecting **OK** will cause the parameters to be saved and you will be returned to the configuration tool’s welcome screen.

![arkime-capture settings confirmation](./docs/images/arkime_confirm.png) ![arkime-capture settings applied successfully](./docs/images/arkime_success.png)
...
Note that only one packet capture engine ([arkime-capture](https://molo.ch/), [netsniff-ng](http://netsniff-ng.org/), or [tcpdump](https://www.tcpdump.org/)) can be used.
...
READ /data/arkime/README.txt and RUN /data/arkime/bin/Configure
...
chown root:netdev /opt/arkime/bin/arkime-capture && \
  setcap 'CAP_NET_RAW+eip CAP_NET_ADMIN+eip CAP_IPC_LOCK+eip' /opt/arkime/bin/arkime-capture

ln -s -f /opt/zeek/bin/zeek /usr/local/bin/
ln -s -f /usr/sbin/netsniff-ng /usr/local/bin/
ln -s -f /usr/sbin/tcpdump /usr/local/bin/
ln -s -f /opt/arkime/bin/arkime-capture /usr/local/bin/
...
>   setcap 'CAP_NET_RAW+eip CAP_NET_ADMIN+eip' /usr/share/packetbeat/bin/packetbeat
root@hedgehog:/tmp# chown root:netdev /opt/arkime/bin/arkime-capture && \
>   setcap 'CAP_NET_RAW+eip CAP_NET_ADMIN+eip CAP_IPC_LOCK+eip' /opt/arkime/bin/arkime-capture
...
root@hedgehog:/tmp# ln -s -f /opt/arkime/bin/arkime-capture /usr/local/bin/
...
* `zeek --version`, `zeek -N local` and `arkime-capture --version` ought to run and print out version information as expected
