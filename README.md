...
* `SURICATA_CUSTOM_RULES_ONLY` – if set to `true`, Malcolm will bypass the default [Suricata ruleset](https://github.com/OISF/suricata/tree/master/rules) and use only user-defined rules in `./suricata/rules`
* `SURICATA_…` - the [`suricata` container entrypoint script](shared/bin/suricata_config_populate.sh) can use **many** more environment variables to tweak [suricata.yaml](https://github.com/OISF/suricata/blob/master/suricata.yaml.in); to list the variables used: `grep -Po "\bSURICATA_\w+" shared/bin/suricata_config_populate.sh | sort -u`
