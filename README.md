...
* `SURICATA_…` - the [`suricata` container entrypoint script](shared/bin/suricata_config_populate.py) can use **many** more environment variables to tweak [suricata.yaml](https://github.com/OISF/suricata/blob/master/suricata.yaml.in); to list the variables used: `grep -Po "\bSURICATA_\w+" shared/bin/suricata_config_populate.py | sort -u`
