...
* **`opensearch.env`** - settings specific to [OpenSearch](https://opensearch.org/)
    ...
    - `OPENSEARCH_PRIMARY` - one of `opensearch-local`, `opensearch-remote` or `elasticsearch-remote`, to determine the [OpenSearch instance](opensearch-instances.md#OpenSearchInstance) Malcolm will use  (default `opensearch-local`)
