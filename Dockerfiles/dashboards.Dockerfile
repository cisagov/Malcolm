...
ENV PUSER "dashboards"
ENV PGROUP "dashboards"
...
RUN amazon-linux-extras install -y epel && \
    ...
    adduser -u ${DEFAULT_UID} -d /home/dashboards -s /bin/bash -G ${PGROUP} -g ${PUSER} ${PUSER} && \
    mkdir -p /usr/share && \
...
ENV PATH "/home/dashboards/.nodenv/bin:${PATH}"

RUN git clone --single-branch --depth=1 --recurse-submodules --shallow-submodules https://github.com/nodenv/nodenv.git /home/dashboards/.nodenv && \
    cd /home/dashboards/.nodenv && \
    ./src/configure && \
    make -C src && \
    cd /tmp && \
    eval "$(nodenv init -)" && \
