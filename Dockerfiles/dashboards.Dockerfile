...
ENV OSD_TRANSFORM_VIS_VERSION 2.11.1
...
RUN yum upgrade -y && \
    ...
    cd /tmp && \
        ...
        sed -i "s/2\.11\.1/2\.12\.0/g" opensearch-dashboards/transformVis/opensearch_dashboards.json && \
        sed -i "s/2\.11\.1/2\.12\.0/g" opensearch-dashboards/transformVis/package.json && \
