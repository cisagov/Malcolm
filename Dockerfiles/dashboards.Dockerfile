...
RUN yum upgrade -y && \
    ...
    cd /tmp && \
        ...
        sed -i "s/2\.8\.0/2\.9\.0/g" opensearch-dashboards/transformVis/opensearch_dashboards.json && \
        sed -i "s/2\.8\.0/2\.9\.0/g" opensearch-dashboards/transformVis/package.json && \
