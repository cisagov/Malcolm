...
RUN chmod 755 /data/*.sh /data/*.py && \
    ...
    ##  sed -i "s/7\.4\.2/7\.5\.2/g" kibana/prelert_swimlane_vis-7.4.2/package.json && \
    ...
    ##   sed -i "s/7\.4\.0/7\.5\.2/g" kibana/kibana-comments-app-plugin/package.json && \
    ...
    ##  sed -i "s/7\.1\.1/7\.5\.2/g" kibana/kibana-milestones-vis/package.json && \
