...
class Metric(object):
    def __init__(self, adapter_id, sensor_id, sensor_key, value, label):
        ...
        if (
            ...
            or label.startswith('Processor')
            or ((label.startswith('Physical') or label.startswith('Package')) and adapter_id.startswith('core'))
        ):
            ...
            try:
                ...
                for hdd_stats in [x.split('|') for x in hdd_temp_line.strip('|').split('||')]:
                    if (len(hdd_stats) == 4) and isfloat(hdd_stats[2]):
                        metrics.append(
                            TemperatureMetric(
                                ' '.join(''.join(filter(lambda x: x in string.printable, hdd_stats[1])).split()),
                                hdd_stats[0],
                                'input',
                                hdd_stats[2],
                                label=LABEL_HDD,
